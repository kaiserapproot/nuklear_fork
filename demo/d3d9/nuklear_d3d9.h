...
NK_API void nk_d3d9_render(enum nk_anti_aliasing AA)
{
    ...
    /* シェーダーを無効化して固定機能パイプラインを使用 */
    hr = IDirect3DDevice9_SetVertexShader(d3d9.device, NULL);
    NK_ASSERT(SUCCEEDED(hr));
    hr = IDirect3DDevice9_SetPixelShader(d3d9.device, NULL);
    NK_ASSERT(SUCCEEDED(hr));
    ...
    /* 頂点フォーマットを設定 */
    hr = IDirect3DDevice9_SetFVF(d3d9.device, D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1);
    NK_ASSERT(SUCCEEDED(hr));

    /* クリッピングを有効化 */
    hr = IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_SCISSORTESTENABLE, TRUE);
    NK_ASSERT(SUCCEEDED(hr));

    /* テクスチャステージの設定 */
    IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_COLOROP, D3DTOP_SELECTARG1);
    IDirect3DDevice9_SetTextureStageState(d3d9.device, 0, D3DTSS_COLORARG1, D3DTA_DIFFUSE);
    ...
    /* クリッピングを無効化 */
    hr = IDirect3DDevice9_SetRenderState(d3d9.device, D3DRS_SCISSORTESTENABLE, FALSE);
    NK_ASSERT(SUCCEEDED(hr));
...
{
    HGLOBAL mem;          /* HGLOBAL型の変数 */
    SIZE_T size;          /* SIZE_T型の変数 */
    LPCWSTR wstr;         /* LPCWSTR型の変数 */
    int utf8size;         /* int型の変数 */
    char* utf8;           /* char*型の変数 */
    ...
    mem = (HGLOBAL)GetClipboardData(CF_UNICODETEXT);  /* 明示的にキャスト */
    if (!mem) {
        CloseClipboard();
        return;
    }

    size = GlobalSize(mem) - 1;  /* メモリサイズを取得 */
    if (!size) {
        CloseClipboard();
        return;
    }

    wstr = (LPCWSTR)GlobalLock(mem);  /* メモリをロックしてデータを取得 */
    if (!wstr) {
        CloseClipboard();
        return;
    }
    ...
    if (utf8size) {
        utf8 = (char*)malloc(utf8size);  /* UTF-8文字列用のメモリを確保 */
        if (utf8) {
            WideCharToMultiByte(CP_UTF8, 0, wstr, (int)(size / sizeof(wchar_t)), utf8, utf8size, NULL, NULL);
            nk_textedit_paste(edit, utf8, utf8size);  /* Nuklearのテキストエディタにペースト */
            free(utf8);  /* メモリを解放 */
        }
    }

    GlobalUnlock(mem);  /* メモリをアンロック */
